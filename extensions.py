...
class Converter:
    ...
    def get_convert(curr_from, curr_to, amount):
        currency = ['AED', 'AFN', 'ALL', 'AMD', 'ANG', 'AOA', 'ARS', 'AUD', 'AWG', 'AZN', 'BAM', 'BBD', 'BDT', 'BGN',
        ...
        if curr_to.upper() not in currency:
            raise APIException(f'Неизвестная валюта {curr_to}')
        if curr_from.upper() not in currency:
            raise APIException(f'Неизвестная валюта {curr_from}')
        try:
            amount = float(amount.replace(',', '.'))
        except ValueError:
            raise APIException(f'Неудалось обработать количество: {amount}')

        url = f'https://api.apilayer.com/fixer/convert?to={curr_to.upper()}&from={curr_from.upper()}&amount={amount}'
        payload = {}
        headers = {"apikey": tok}
